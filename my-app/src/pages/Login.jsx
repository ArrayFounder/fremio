...
export default function Login() {
  ...
  // Load Google Sign-In (manual OAuth redirect flow)
  useEffect(() => {
    const clientId = import.meta.env.VITE_GOOGLE_CLIENT_ID;
    if (!clientId) return;

    // Handle OAuth redirect: check for code in URL on page load
    const params = new URLSearchParams(window.location.search);
    ...
    if (code) {
      ...
      handleGoogleRedirectCallback(code);
      return;
    }
  }, []);
  ...
  const handleGoogleSignIn = () => {
    ...
    const redirectUri = encodeURIComponent('https://fremio.id/auth/google/callback');
  ...
  // Handle Google OAuth redirect callback (exchanges code for user data)
  const handleGoogleRedirectCallback = async (code) => {
    setGoogleLoading(true);
    setError('');

    try {
      // Exchange code + state for user data
