...
import { THEMES, STICKERS, getIconUrl } from "@/config/stickerCatalog";
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {[{ label: "4R", ratio: "2:3", desc: "1200 × 1800", icon: "🎞️", matchRatios: ["2:3"] }].map((preset) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      border: isSelected ? '2px solid #d4a99a' : '1px solid rgba(224, 183, 169, 0.15)',
                      background: isSelected ? 'linear-gradient(135deg, #fdf7f4 0%, #f7ebe5 50%, #f1dfd6 100%)' : 'linear-gradient(135deg, #ffffff 0%, #fefcfb 100%)',
                      cursor: 'pointer', textAlign: 'left', transition: 'all 0.25s ease',
                      boxShadow: isSelected ? '0 4px 16px rgba(212, 169, 154, 0.25), inset 0 1px 0 rgba(255,255,255,0.8)' : '0 2px 8px rgba(224, 183, 169, 0.08)',
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <div>
                          <div style={{ fontWeight: isSelected ? 700 : 600, fontSize: '14px', color: isSelected ? '#4a302b' : '#5a3d38', marginBottom: '2px' }}>{preset.label}</div>
                          <div style={{ fontSize: '12px', color: isSelected ? '#c89585' : '#9ca3af', fontWeight: 500 }}>{preset.desc}</div>
                        </div>
                      </div>
                      {isSelected && (
                        <div style={{ width: '24px', height: '24px', borderRadius: '50%', background: 'linear-gradient(135deg, #e0b7a9 0%, #d4a99a 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 8px rgba(212, 169, 154, 0.4)' }}>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <button
                    key={grid.label}
                    type="button"
                    onClick={() => onConfirmAddPhoto(grid.rows, 2)}
    ...
    >
      {showFontPicker ? renderFontPicker() : (
        <>
          ...
          {/* Dimensi - tidak untuk teks, background-photo, dan photo */}
